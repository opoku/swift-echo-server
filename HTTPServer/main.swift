...
//  HTTPServer
//
//  Created by Osei Poku on 7/23/15.
...
class Server: SessionDelegate {
    ...
    init() {
        ...
        register_signal_handler(SIGINFO) { () -> Void in
            self.logSessions()
        }
    }
    ...
    func listen () -> Bool {
        ...
        let result = withUnsafePointers(&sin4, &sin6, &socketContext) { sinPtr, sin6Ptr, ctxtPtr -> Bool in
            ipv4sock = CFSocketCreate(kCFAllocatorDefault, PF_INET, SOCK_STREAM, IPPROTO_TCP, CFSocketCallBackType.AcceptCallBack.rawValue, acceptConnection, ctxtPtr)
            ipv6sock = CFSocketCreate(kCFAllocatorDefault, PF_INET6, SOCK_STREAM, IPPROTO_TCP, CFSocketCallBackType.AcceptCallBack.rawValue, acceptConnection, ctxtPtr)
            var yes = 1
